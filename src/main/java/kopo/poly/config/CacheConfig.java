...
/**
 * Spring Boot 3 환경에서 JCache(JSR-107) + Ehcache 3을
 * 스프링 캐시 추상화에 연결하는 설정 클래스.
 * <p>
 * - @EnableCaching : @Cacheable/@CacheEvict 등 어노테이션 활성화
 * - Encache JSR-107 Provide를 명시적으로 선택하여 JCache 표준 API로 캐시글 사용
 * - "weather" 캐시글 생성하고 TTL을 10분으로 지정
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public org.springframework.cache.CacheManager cacheManager() {
        ...
        //      -FQCN을 지정해 Ehcache 의 JSR-107 Provider를 직접선택
        //      -클래스명 : org.ehcache.jsr107.EncacheCachingProvider
        ...
        // 2) 표준 JCache CacheManager 열기
        //      - classpath 내 encache.xml 등 외부 설정이 있으면 provider가 읽어들일 수 있음
        ...
        // 5) 스프링 캐시 추상화 어댑터로 감싼 반환
        //    - 이후 서비스 메서드에서 @Cacheable(cacheNames = "weather") 등으로 사용 가능
        //    - JCacheCacheManager는 내부적으로 jcacheManager를 주입 사용
