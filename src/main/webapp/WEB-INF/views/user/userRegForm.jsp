...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            //우편번호 찾기
            $("#btnAddr").on("click", function () { //버튼 클릭했을때, 발생되는 이벤트 생성함(onclick 이벤트와동일함)
                kakaoPost(f)
            })
            //회원가입
        ...
        function doSubmit(f) {
            ...
            if (userIdCheck !== "N") {
                alert("아이디 중복 체크 및 중복되지 않은 아이디로 가입바랍니다.");
                f.userId.focus();
                return;
            }
            ...
            $.ajax({
                    ...
                    datatype: "JSON", // 전송 결과는 JSON 으로 받기
                    data: $("#f").serialize(), // form 태그 내 input 등 객체를 자동으로 전송할 형태로 변경하기
                    success: function (json) {

                        if (json.result === 1) { //회원가입성공
